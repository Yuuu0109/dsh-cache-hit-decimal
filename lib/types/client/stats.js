 ...
 */
export function formatCacheHitPercent(cacheReadTokens, promptTokens, decimalPlaces = 2) {
    if (promptTokens === 0)
        return null;
    if (cacheReadTokens >= promptTokens)
        return '100';
    const units = roundedPercentUnits(cacheReadTokens, promptTokens, decimalPlaces);
    if (units < 10 ** decimalPlaces * 100)
        return displayPercentUnits(units, decimalPlaces);
